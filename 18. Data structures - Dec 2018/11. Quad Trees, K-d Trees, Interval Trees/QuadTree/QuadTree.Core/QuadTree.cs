    ...
    public class QuadTree<T> where T : IBoundable
    {
        ...
        private void TrySplitNode(Node<T> node, int depth)
        {
            ...
            if (node.Children == null)
            {
                ...
                node.Children[0] = new Node<T>(node.Bounds.MidX, y1, rightWidth, upperHeight); //upperRight
                node.Children[1] = new Node<T>(x1, y1, leftWidth, upperHeight); //upperLeft
                node.Children[2] = new Node<T>(x1, node.Bounds.MidY, leftWidth, lowerHeight); //lowerLeft
                node.Children[3] = new Node<T>(node.Bounds.MidX, node.Bounds.MidY, rightWidth, lowerHeight); //lowerRight
            }
